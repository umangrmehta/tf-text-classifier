...
def get_model(input_layer, embedding_layer, model_name, classes=2):
	...
	lstm = tf.keras.layers.LSTM(32, return_sequences=True)(embedding_dropout)
	conv = tf.keras.layers.Conv1D(64, 5, activation='relu')(lstm)
	pool = tf.keras.layers.MaxPooling1D(pool_size=4)(conv)
	pred, loss = architecture.get_output_layer(pool, classes)
